...
public interface TimetableStudentRepo extends JpaRepository<TimetableStudent, UUID> {
    ...
    @Query(nativeQuery = true, value = """
            select tts.id as timetableStudentId, u.phone_number as phoneNumber, array_agg(sa.attendance order by sa.lesson_order) as attendances, array_agg(sa.lesson_date order by lesson_order) as lessonDates from timetable_student tts
